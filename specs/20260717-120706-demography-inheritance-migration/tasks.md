...
- [ ] T043 Extend §4.1 of `docs/whitepaper/epocha-whitepaper.md` AND `docs/whitepaper/epocha-whitepaper.it.md` with the inheritance and migration models: the Falconer & Mackay kernel, the per-trait heritability table with its primary sources, the four social-class rules, the five succession rules with their sources, the estate-tax rates, the Harris-Todaro operational variant with its declared simplification, Mincer family coordination, and emergency flight with the I-5 constraint. State every simplification and what it loses (FR-030, FR-032). Acceptance: both languages carry equivalent content; every author-year cited has a §13 bibliography entry.
