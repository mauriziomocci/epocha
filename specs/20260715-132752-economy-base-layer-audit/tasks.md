...
- [x] T001 [A] RED test in `epocha/apps/economy/tests/test_production.py` (create if absent): `test_ces_leontief_limit_continuity` — for alpha=[0.9,0.1], X=[1.0,1.0], assert the general CES value at sigma just above the Leontief threshold (e.g. 0.06) and the Leontief-branch value at sigma below it (e.g. 0.04) agree to within a small epsilon (continuity across sigma=0.05), and that both equal min(X)=1.0 (NOT min(alpha*X)=0.1). Fails today (returns 0.1). Also `test_ces_leontief_equals_min_inputs` — Leontief branch returns A*min(X_i).
- [x] T002 [A] Fix `production.py:87-92`: Leontief limit returns `scale * min(x for _, x in pairs)` (i.e. A*min(X_i)), matching the ACMS limit of the normalized aggregator; update the docstring comment to state the true limit min(X_i) and that distribution weights vanish in the limit. GREEN T001.
- [x] T003 [A] Fix `production.py:190,199,201`: tag the 0.5 capital/natural_resources/knowledge baselines as tunable design parameters in the inline comments (matching the labor/scale tagging style), or cite a source. No behavior change; existing tests stay green.
- [x] T004 [A] Gates + commit `fix(economy): correct CES Leontief limit and tag production baselines`.

## Group B — Monetary documentation fixes (monetary.py, isolated)

- [x] T005 [B] Fix `monetary.py:136`: docstring "Below poverty: linear mood penalty" → "Below poverty: flat penalty (step function, not scaled by wealth)"; the code is a deliberate step and the docstring must match (Code Comments rule). No behavior change.
