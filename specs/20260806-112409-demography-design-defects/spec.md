...
- **SC-017** *(nuovo, copre l'accoppiamento assortativo)* — la correlazione fra i genitori sul carattere trasmesso è misurata e riportata, e SC-002b usa il bersaglio corretto per essa.
...
- **SC-019** *(nuovo, copre FR-006)* — il fattore di annualità eguaglia `(1 − e^{−r_anno·H_anni})·(8760/tick_duration_hours)/r_anno` entro `1·10⁻¹²`, per **almeno due valori distinti** di `tick_duration_hours`. Il criterio è sul **fattore**, non sul guadagno: il guadagno non è invariante al cambio di durata del tick, quindi un criterio di invarianza certificherebbe la mutazione che fissa 24 ore nel codice e boccerebbe l'implementazione corretta.
