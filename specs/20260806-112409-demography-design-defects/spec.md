...
**IL DIFETTO È PIÙ AMPIO DI COME LA PRIMA STESURA LO DESCRIVEVA, ma il secondo giro l'ha descritto male a sua volta.** I meccanismi di trasmissione della forma "media pesata fra un valore parentale e un riferimento" sono **cinque**, non quattro, e si dividono così:

| meccanismo | termine di innovazione | difetto |
|---|---|---|
| kernel poligenico dei tratti | **sì** | il residuo è scalato male: fornisce 0,0675 dove ne servirebbero 0,1382 |
| regressione dell'istruzione | **no** | contrazione deterministica: collassa a un punto |
| regressione di classe alla Clark | **no** | deterministica: si congela in una generazione |
| mobilità alla Becker-Tomes | **sì** | nessuno trovato |
| regola meritocratica | no | **ereditato, non indipendente** |

Due meccanismi possiedono un termine di innovazione, non uno solo; e il kernel poligenico è fra questi. La stesura precedente scriveva che solo Becker-Tomes ne ha uno, il che contraddiceva i propri stessi numeri: un meccanismo senza nulla che rigeneri la dispersione va a **zero**, come infatti fa l'istruzione, mentre il kernel si assesta al 49% proprio perché qualcosa la rigenera. La regola meritocratica, poi, non calcola una media fra un genitore e un riferimento: deriva la classe dai tratti già ereditati del figlio, quindi una volta riparati istruzione e intelligenza la sua distribuzione torna non degenere da sola, e chiederne una correzione separata sarebbe sovradimensionare l'ambito.
...
3. **Given** un template privo di una sezione che il design dichiara obbligatoria, **When** lo si carica, **Then** il caricamento fallisce anziché ricadere silenziosamente su un valore di ripiego.
...
- **FR-002**: la dispersione stazionaria dei tratti prodotta dal kernel poligenico MUST eguagliare l'ampiezza d'era dichiarata, entro una tolleranza che la spec emendata MUST fissare **numericamente** e che MUST NOT contenere il valore attuale. Questo è il vincolo portante dell'intero work item e la stesura precedente lo aveva perso: misurato, il kernel odierno si assesta al **49,0%** dell'ampiezza dichiarata.
- **FR-002b**: **ogni altro** meccanismo di trasmissione intergenerazionale MUST possedere un termine di innovazione e una distribuzione stazionaria non degenere. Il requisito copre la regressione dell'istruzione e la regressione di classe, che oggi ne sono privi.
...
- **SC-011**: ciascun meccanismo di trasmissione, fatto evolvere per almeno otto generazioni ai parametri spediti, produce una dispersione stazionaria **almeno pari al 50% di quella della popolazione iniziale**. Misurato oggi: l'istruzione collassa a 0,00004 e a zero netto, e la classe alla Clark si congela in una generazione su una partizione deterministica immobile. Una soglia formulata come "distribuzione non degenere" NON discrimina — verificato: la superano oggi il kernel poligenico, la regressione di classe e la regola meritocratica, e la fallisce la sola istruzione.
