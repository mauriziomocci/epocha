...
## Phase-2 adversarial review, round 4 — NOT CONVERGED narrowly, remediated

Round 4 opened by doing the one thing that matters at this gate: taking every requirement and criterion in the document and checking it against the **current unrepaired code**, mechanism by mechanism. A requirements document for a correction must fail against the thing being corrected, and round 3 had found that none of it did. This time **nine of the ten defects are bound by something that is false today** — a real improvement, and the reviewer said so plainly rather than manufacturing a verdict.

The tenth is what blocked convergence, and it is the same error one more time, in the criterion written to fix it.

**A single percentage floor was applied across incommensurable scales.** SC-011 asked every mechanism for a stationary dispersion of at least half its initial value. Independently reproduced: the Clark class regression measures **90.8% of the founder dispersion** — clearing the floor with forty points to spare — while its intergenerational mobility is **exactly 0.0000** from the second generation onward, on a partition that stays bit-identical for eight more. So the criterion certified as sound the very mechanism whose defect the requirement beside it exists to catch. In the opposite direction the same floor condemned the meritocratic rule, which measures 33% today and would still measure 38% **after every correction this work item mandates**, because merit is the mean of two narrow [0,1] variates projected onto a rank scale whose reference dispersion comes from a wealth-percentile partition that rule never touches — an unreachable floor and a toothless criterion in one line, jointly unsatisfiable with the requirement that the suite stay green.

The remedy is to split by the kind of quantity being governed: a **dispersion** floor for the continuous characters, traits and education, and an **intergenerational-mobility** floor for the rank-valued class rules. Measured, mobility discriminates exactly where dispersion does not — zero for Clark, 0.58 for Becker-Tomes.

**The companion requirement over-reached onto two mechanisms it should never have bound.** As written it demanded an innovation term from *every* other transmission mechanism, which reaches the rigid patrilineal class succession — the rule of both pre-industrial eras, cited to sources precisely for its rigidity, where imposing mobility would contradict the citation — and the meritocratic rule, which the same document exempts two paragraphs earlier. The mechanism table was also short by one: there are **six** transmission mechanisms, of which only three are in scope. All three now carry an explicit in-scope column.

**A census error was corrected, and it matters for honesty rather than for planning.** The document opened by stating that all the items in scope are already declared in the whitepaper as true of the model, so the work item closes rather than discovers. That is true of the original eight and false of the two the gate rounds themselves added: the template-loader defect is the *opposite* — §6.2 publishes as true a property the loader does not have — and zone stability sits outside the deferred set. The text now distinguishes the two statuses.

Smaller repairs: three stale counts left over from the previous expansion; two figures for one measured quantity, where the round-3 remediation had introduced 49.0% against the analytic 48.845% it endorses elsewhere; the acceptance scenario that still asked for the realised-heritability test the criteria record as a trap, now removed; four requirements that carried no measurable criterion, now covered; the conservation rates now reported with the estate-value distribution they were measured under, since they move by a couple of points across distributions; the requirement's referent clarified, since the code describes the same parameter two ways that differ by a factor of 0.921 under the published corrective form; and the loader scenario now names the nesting level, because a template missing a *top-level* section is already rejected today and a scenario that did not say so would not have failed against the code.
