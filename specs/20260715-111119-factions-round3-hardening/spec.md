...
- **FR-004** (NEW-13): `compute_leadership_score` DEVE accettare un parametro opzionale con i membri del gruppo già fetchati (default: fetch autonomo, comportamento attuale) e dati relazionali prefetched; `compute_legitimacy` e `update_group_leadership`/`_elect_new_leader` DEVONO passare i membri fetchati una sola volta. Budget di query per gruppo costante rispetto al numero di membri. Valori numerici invariati (equivalenza testata). AMENDMENT fase 5-6 (escalation recepita): la stessa classe N+1 esiste anche nell'elezione dei fondatori dentro `_create_faction` e viene chiusa nello stesso branch con lo stesso pattern (members=founders + contesto leadership, memorie fondatori in bulk_create, budget pinnato). La correzione ha rivelato e sanato un difetto comportamentale preesistente DICHIARATO: su develop l'elezione valutava i candidati contro il gruppo appena creato ancora VUOTO (ogni fondatore scoreva 0.0 e vinceva sempre `founders[0]` per stable sort); ora valuta l'effettiva membership fondativa — regression test comportamentale dedicato che asserisce l'identità del leader eletto. Nota per il budget costante di SC-002: il loop di memorie di transizione della successione (`Memory.create` per membro su cambio leader) è O(membri) per-EVENTO, non per-tick, e resta fuori dal budget costante come deferral dichiarato (stessa classe delle memorie di schism/dissoluzione).
