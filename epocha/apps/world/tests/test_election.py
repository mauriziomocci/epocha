...
def user(db):
    return User.objects.create_user(
        email="elec@epocha.dev", username="electest", password="pass123"
    )
...
def government(simulation):
    return Government.objects.create(
        simulation=simulation, government_type="democracy", last_election_tick=0
    )
...
def factions_and_agents(simulation):
    faction_a = Group.objects.create(
        simulation=simulation, name="Reformers", objective="Reform", cohesion=0.7, formed_at_tick=1
    )
...
class TestRunElection:
    ...
    def test_election_creates_public_memory(
        self, simulation, world, government, factions_and_agents
    ):
        run_election(simulation, tick=50)
