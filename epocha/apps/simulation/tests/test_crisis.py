...
def user(db):
    return User.objects.create_user(
        email="crisis@epocha.dev", username="crisistest", password="pass123"
    )
...
def snapshot_stable(simulation):
    return SimulationSnapshot.objects.create(
        ...
        corruption=0.2,
        popular_legitimacy=0.6,
        military_loyalty=0.6,
        population_alive=20,
    )
...
class TestDetectCrises:
    ...
    def test_creates_public_memory_on_crisis(
        self, simulation, world, government, snapshot_inequality
    ):
        Agent.objects.create(
            simulation=simulation, name="Marco", role="citizen", personality={"openness": 0.5}
        )
        detect_crises(simulation, snapshot_inequality)
        memories = Memory.objects.filter(source_type="public", content__contains="EPOCHAL CRISIS")
        assert memories.count() >= 1

    def test_cooldown_prevents_duplicate_crisis(
        self, simulation, world, government, snapshot_inequality
    ):
        detect_crises(simulation, snapshot_inequality)
    ...
    def test_multiple_crises_can_fire_simultaneously(self, simulation, world, government):
        snap = SimulationSnapshot.objects.create(
            ...
            tick=10,
            gini_coefficient=0.7,
            government_stability=0.2,
            avg_mood=0.2,
            institutional_trust=0.15,
            corruption=0.7,
            popular_legitimacy=0.15,
            military_loyalty=0.5,
            population_alive=20,
        )
        crises = detect_crises(simulation, snap)
