...
def marco(simulation):
    return Agent.objects.create(
        simulation=simulation, name="Marco", role="blacksmith", personality={"openness": 0.5}
    )
...
def elena(simulation):
    return Agent.objects.create(
        simulation=simulation, name="Elena", role="farmer", personality={"openness": 0.5}
    )
...
class TestUpdateReputation:
    ...
    def test_negative_hearsay_decreases_reputation(self, marco, elena):
        update_reputation(
            holder=elena, target=marco, action_sentiment=-1.0, reliability=0.7, tick=5
        )
        score = ReputationScore.objects.get(holder=elena, target=marco)
        assert score.reputation < 0.0
    ...
    def test_low_reliability_has_less_impact(self, marco, elena):
        update_reputation(
            holder=elena, target=marco, action_sentiment=-1.0, reliability=0.1, tick=5
        )
        ...
        update_reputation(
            holder=elena, target=marco, action_sentiment=-1.0, reliability=0.9, tick=5
        )
