...
def world(simulation):
    return World.objects.create(
        simulation=simulation, distance_scale=133.0, tick_duration_hours=24.0
    )
...
def versailles(world):
    return Zone.objects.create(
        ...
        name="Versailles",
        zone_type="urban",
        boundary=Polygon.from_bbox((0, 0, 120, 120)),
        center=Point(60, 60),
    )
...
def paris(world):
    return Zone.objects.create(
        ...
        boundary=Polygon.from_bbox((150, 0, 270, 120)),
        center=Point(210, 60),
    )
...
def campagna(world):
    return Zone.objects.create(
        ...
        name="Campagna",
        zone_type="rural",
        boundary=Polygon.from_bbox((150, 150, 270, 270)),
        center=Point(210, 210),
    )
...
class TestCalculateMaxDistance:
    def test_carriage_travels_farther_than_foot(self, world, government):
        dist_carriage = calculate_max_distance(
            "carriage", health=1.0, world=world, government=government
        )
    ...
    def test_low_health_reduces_distance(self, world, government):
        dist_healthy = calculate_max_distance(
            "foot", health=1.0, world=world, government=government
        )
    ...
    def test_high_repression_reduces_distance(self, world, simulation):
        ...
        World.objects.create(
            simulation=gov_high_sim, distance_scale=133.0, tick_duration_hours=24.0
        )
        ...
        dist_high = calculate_max_distance(
            "foot", health=1.0, world=world_high, government=gov_high
        )
...
class TestExecuteMovement:
    ...
    def test_movement_to_far_zone_is_partial(
        self, agent_at_versailles, campagna, world, government
    ):
