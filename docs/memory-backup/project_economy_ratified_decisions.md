...
- **Test di determinismo**: per i bug di ordine-DB un RED affidabile e' spesso impossibile (Postgres restituisce i pari-merito in ordine di inserimento, che coincide con l'id). Quei test **pinnano il contratto deterministico** invece di riprodurre il non-determinismo, ed e' dichiarato nella docstring. Eccezione: `test_sell_property_asking_price_deterministic_on_expectation` e' un RED reale.
- **R12-DET-1 FUORI SCOPE** (ratificato): i resolver `pair_bond`/`separate`/`move_to` a `simulation/engine.py:148-171` usano `.first()` non ordinato. Difetto reale ma di dominio demografia/agents, che ha il suo §4.1 con frozen-pin proprio. Work item separato -- vedi [[post-mvp-roadmap]] e la build map.
