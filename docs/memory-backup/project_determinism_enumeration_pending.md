...
metadata: 
  ...
  modified: 2026-07-20T13:03:50.897Z
---
...
4. **Anche il rimedio ha un limite: `get_seeded_rng` semina sulla CHIAVE PRIMARIA, non solo sul seme** (verificato 2026-07-20, T040 di demografia Plan 3). La chiave e' `f"{simulation_id}:{base_seed}:{tick}:{phase}"` (`demography/rng.py:41-42`, e con namespace `"economy:"` a `economy/rng.py:53`). Quindi riprodurre un risultato pubblicato su un database pulito da un `simulation.id` diverso e **flussi RNG diversi**, a parita' di seme dichiarato: neanche la parte non-LLM e' riproducibile fuori dal database originale. E' deliberato, non un bug -- `economy/rng.py` lo dichiara correttamente nel proprio docstring e ha superato il fix R7-RNG-1 del Round 7 -- ma **il docstring di `demography/rng.py` lo NEGA**, promettendo "publication-grade reproducibility" da un hash di "(simulation.seed, tick, phase)" senza mai nominare l'id. Nessuno dei due documenta la conseguenza sulla riproducibilita', che e' quella che il paper deve dichiarare. Un test permanente pinna l'accoppiamento: `TestGetSeededRngSimulationIdCoupling` in `demography/tests/test_inheritance.py`. Il punto 2 sopra indica `get_seeded_rng` come "il rimedio": lo e', ma con questo limite.
