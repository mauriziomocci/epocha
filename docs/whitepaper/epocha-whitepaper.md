...
**The survival horizon, shared with §4.1.5** (amendment A8). `wealth / subsistence_threshold` is a pure number saying how many ticks of subsistence an agent's savings cover, and it is a DERIVED quantity: there is no global threshold `N` on it anywhere in the model. The design document used to define one as "the number of ticks the agent can survive on current savings", which reduces the condition it appeared in to `wealth < wealth` — a quantity confused with a threshold on it. Each consumer now declares its own with its own reason. Fertility consumes the horizon continuously through the logarithm of (4.3), and the `0.1` inside that logarithm is a NUMERICAL GUARD against `log(0)` for a destitute agent, capping the signal at `log(0.1) = −2.30` instead of negative infinity; it is not a modelled floor on the horizon and must not be read as a calibrated parameter. Emergency flight declares the threshold 1 and leaves duration to its own per-era `flight_trigger_ticks`. Neither consumer's behaviour changed when this was corrected: the incoherence was entirely in the design document.
...
**Model.** The polygenic kernel is `inherit_trait()`:
...
Equation (4.46) is the variance-preserving form of the Falconer and Mackay (1996, chs. 8-10) decomposition, and the three residual scales are **derived from one identity rather than chosen**. Requiring the offspring variance to equal the parental variance `V` under random mating, with `Var(parent_term) = V/2` at two parents and `V` at one, gives
...
Clamping to `[0, 1]` truncates the distribution, so the realized dispersion is below the declared `era_sd` even under the exact identity. How far below depends on where the pair sits: the deterministic grid fixed point in `truncated_moments.py` puts the stationary amplitude at **99.91%** of the declared value at the centred trait pair `(0.50, 0.15)` and at **97.72%** at the off-centre education pair `(0.30, 0.15)`, which also carries 2.14% of its mass on the bounds. Those figures are measured and reported at template load, not gated — a band on realized amplitude cannot separate a correct kernel from one whose residual is mis-scaled, which is precisely the failure the exact scale tests above exist to catch. The floor the loader does enforce is 95%, on the worst of the three kinship branches rather than on a conventional one, because measured over a grid of configurations the two-parent branch fails to be the minimum in a majority of cases.

**The identity assumes random mating, and the model does not provide it.** `Var(midparent) = V/2` holds only for uncorrelated parents, and the homogamy score weights education between 0.25 and 0.40 in all five eras — so restoring education's dispersion is exactly what wakes that correlation up, everywhere rather than only where a class rule reads a trait. With parents correlated at `r` the midparent variance is `V(1+r)/2` and the stationary amplitude inflates to `√((1 − b²/2) / (1 − b²(1+r)/2))` of the declared value. On the untruncated recursion that leaves the 95-105% band past `r = 0.5217` for the traits and `r = 0.4235` for education; **truncation inverts the order**, moving the crossings to `r = 0.5403` and `r = 0.7122`, because the off-centre education pair must first make back the 2.28% the clamp already costs it. Those four numbers are reported alongside the measured `r`, never gated: their purpose is to make visible how far mating has moved the variance, not to reject a template. The mating convention is declared rather than left to the implementer — a Gaussian copula on the current marginal, with the realized Pearson correlation on the observed `[0, 1]` scale as the reported quantity, since mass the clamp piles on a bound is tied and carries no covariance, so the copula parameter would overstate the assortment. Two implementations picking different conventions would publish different thresholds for the same era, which is why the choice is fixed here.
...
shari'a       : widower 1/4 with children, 1/2 without;
                widow (and non-binary spouse) 1/8 and 1/4
...
**Simplifications.** Four rounds of adversarial code audit converged on 2026-08-05 over `inheritance.py`, `migration.py`, and the `couple.py` change of §4.1.3. That verdict covered the code as scoped and explicitly did not cover the DESIGN-level defects, which were deferred to a separate work item with its own requirements gate. That work item has since landed the corrections below; each is stated with what it replaced, because a chapter that quietly swapped one claim for another would leave every result published under the old one unmarked.

**Results produced before and after these corrections are not comparable.** The trait kernel, the education regression, the class rule of the industrial era, the spousal share of the Islamic era and four of the five education coefficients all changed. Any multi-generation figure predating them describes a different model.

*The polygenic kernel is now variance-preserving.* Until August 2026 equation (4.46) computed a convex combination rather than the Falconer and Mackay decomposition it cited, and trait spread collapsed to 48.8% of the declared era distribution within roughly three generations and stayed there — every simulated society drifting toward homogeneity on all thirteen heritable traits of Table 4.10, with realized heritability no longer matching the cited figures after the first generation. The three residual scales now follow from the variance identity, and the single-parent branch applies `h²/2` rather than the full `h²`: the old branch returned a bit-identical value for two parents at 0.9 and mother-only at 0.9, an implemented resemblance twice the cited model's.

*The shari'a spouse fraction is gendered.* Q4:12 is asymmetric — the widower takes 1/2 without a child and 1/4 with, the widow 1/4 and 1/8 — and the implementation applied the widow's schedule to both partners, so a surviving husband received half his classical entitlement in both branches. The fractions are now read from the primary text; Powers (1986) remains the source of the surrounding fixed-share-plus-residuary structure and of the 2:1 ratio of Q4:11, both verified correct, and was never a source for the gender-blind share.

*Estate tax and remainder conserve exactly.* Equation (4.51) used to multiply twice rather than derive one term by subtraction, and exact conservation `tax + remainder == total` failed on 16.1% of estates at rate 0.15 and 6.0% at 0.40, at a relative error of 1.9e-16. It now derives the SMALLER term by subtraction on either side of a rate of one half, which is the condition of Sterbenz's lemma, and the property holds bit-for-bit over the whole rate domain — verified at zero failures over 20,000 estates at each of eleven rates, with the tax read from the treasury rather than re-derived. The guarantee names its summation order: `inheritable + tax_revenue`. Deriving the remainder alone does not reach exactness even at the shipped rates; deriving the tax alone is exact only up to 0.5 and then degrades to 12.68% of trials at 0.70.

*The education coefficients trace to a source that exists.* The design attributed the modern value to Chetty et al. (2014). Both 2014 papers were retrieved in full and searched: neither reports an education-persistence coefficient of any value. The one that measures education reports a gradient against INCOME RANK, 74.5% and 69.2%, not against education. The value was removed rather than relocated — the trap worth naming is that Sacerdote (2000)'s maternal coefficient is also 0.35, but it is a single-parent estimate and it is not Chetty. The same paper's elasticity range was also cited backwards: its measured values run 0.264 to 0.697 and its thesis is that the elasticity is unreliable, so `becker_tomes` now attributes 0.4 to Solon (1999) alone, with its verification status declared UNVERIFIED because that chapter is paywalled and unread.
...
a(H, r) = (1 − e^(−r·H)) / r          [ticks]
H       = max(0, 62 − age) · ticks_per_year,  ticks_per_year = 8760 / tick_duration_hours
r       = 0.10 / ticks_per_year
```

Equation (4.57) is a **discounted present value**, and the sources behind it separate cleanly. Harris and Todaro (1970) is a one-period equilibrium equality, `W_u·E_u/L_u = W_R`, with no horizon, no discounting and no cost: it is the source of the employment-probability weighting on the destination wage and of nothing else — in particular it cannot license a horizon. Todaro (1969) states the decision as `V(0) = Σ [p(t)·Y_u(t) − Y_r(t)]·e^(−it) − C(0)`, a lump-sum cost subtracted from a discounted flow rather than netted against a rate. Sjaastad (1962) supplies both the horizon — residual working life, which shortens with age — and the definition of the cost: outlays plus earnings foregone while travelling and searching, partly a function of distance. `62` is not chosen: it is the age at which Sjaastad's two published horizons, 45 remaining years for his 15-19 bracket and 40 for 20-24, both fall out of the bracket midpoints. The discount rate of 10% per annum is his stated assumption (p.92), declared tunable; at a tick that is a year the annuity reproduces the two factors he prints on p.89, 9.889 and 9.817.

Until August 2026 the equation was `(1 − unemployment_j) · wage_j − wage_current − distance_cost_j`, which subtracts a **count of ticks** from a **money per tick**. The design's own worked example does not expose it, because the destination it computes has a distance cost of zero. Monetising the cost alone does not fix it either: that leaves one money against two rates. Two departures from the canonical two-sector model remain documented rather than silent: the informal-sector wage is set to zero, so an agent who fails to find formal work at the destination earns nothing there, and a distance term is carried that the two-sector model does not have.

`build_migration_outlook()` (`migration.py:456`) assembles the per-agent prompt block from a once-per-tick `zone_stats` bundle, issuing zero database queries of its own. Every reachable zone — defined as every zone of the world other than the agent's own, with no radius bound, because the schema carries no maximum-travel-range concept and (4.56) already prices distance — receives an entry carrying `wage_differential`, `unemployment`, `distance_cost` and `expected_gain`, with `government_stability` reported once at outlook level. Household coordination is `coordinate_family_migration()` (`migration.py:681`): the deciding agent's living partner and every living child below the era's `adulthood_age` are moved into the target zone in the same tick, with `location` written alongside `zone` in one `bulk_update`, and one `DemographyEvent` of type `migration` is emitted for the whole household rather than one per member. Adult children are deliberately excluded — they decide independently — and no minor ever receives a `DecisionLog` row for a move they did not choose.
...
**Simplifications.** The two design-level defects the converged code audit left open have both been corrected by the deferred work item, and **results produced before and after are not comparable** — the expected gain changed by three orders of magnitude and the trapped-agent verdict changed with it.

*The expected gain is a present value.* The old form was dimensionally inconsistent, and the consequence of fixing it is large and is a loss, so it is stated rather than buried. At 24-hour ticks, a 40-year horizon and 10% per annum, `a` is 3583 ticks, so the design's worked example of +4.8 per tick becomes a present value of +17,199. The break-even distance cost moves from 4.8 ticks to 220.5, against shipped costs of 0, 3 and 5: **distance stops biting**, and the threshold widens by roughly a factor of forty-five. This is what the cited model implies, and its own author says so — Sjaastad observes (p.84) that marginal costs per mile would have to be implausibly high to reconcile the observed distance effect with the present value of the differential, "even at very high discount rates". The investment model under-predicts the friction of distance.

*Migration stops entirely at 62.* The horizon reaches zero rather than tapering, so from `WORKING_LIFE_END_AGE` on the annuity is exactly 0.0 and (4.57) collapses to `−distance_cost_ticks · wage_current`: zero at a co-located destination and strictly negative otherwise, no matter how large the wage differential. Because the flight path rejects a non-positive gain, an agent at or past that age is permanently immobile and cannot flee starvation toward a zone paying a thousand times its own. Measured: the factor is 347.3 ticks at 61 and 0.0 at 62. This is arithmetically correct under Sjaastad's investment framing — someone with no working life left recovers nothing from a move — and it is a sharp boundary that no source endorses at that exact age. The `max(0, ·)` in the horizon above is load-bearing and not decoration: without it a seventy-year-old would carry a negative annuity and be PAID to migrate for having grown old. The natural refinement, deferred rather than invented, is a non-labour motive for late-life migration that does not run through the wage differential at all; the cited model has nothing to say about one.

*Only one of Sjaastad's two cost components is modelled.* His cost is outlays PLUS earnings foregone; (4.57) prices only the second, because `compute_distance_cost` returns ticks and nothing in the schema prices a journey in currency. The consequence is sharp at the bottom of the distribution: an agent whose zone has no wage data has `wage_current = 0`, so travel is free and no destination is ever rejected on distance. Correct under the modelled component — someone earning nothing forgoes nothing by leaving — and wrong under the unmodelled one, which is exactly the population an out-of-pocket fare binds hardest. This was found by rebuilding the module's own trapped-agent fixtures against the new equation, not by inspection.

*The flight trigger's threshold is now derived rather than global.* The **survival horizon** is `agent.wealth / compute_subsistence_threshold(simulation, zone)`, a pure number saying how many ticks of subsistence an agent's savings cover. There is no global `N`: the design used to define one as "the number of ticks the agent can survive on current savings", which reduces the condition it appeared in to `wealth < wealth`, confusing a quantity with a threshold on it. Each consumer declares its own. Emergency flight applies the threshold **1** — inability to cover even one tick — and leaves duration to its own per-era `flight_trigger_ticks`, which ships at 30, 30, 20, 10 and 5: imposing a second horizon on the level as well would count the same duration twice, with the second occurrence unsourced. Fertility consumes the horizon continuously in logarithmic form with a floor of 0.1 that is a numerical guard against `log(0)`, not a modelled floor. Neither consumer's behaviour changed; the incoherence was entirely in the design document.
...
| Inheritance (polygenic kernel + social transmission + succession) | Tables 4.10 (per-trait heritability with primary sources) and 4.11 (per-era class rule, education-regression ρ, succession rule, estate-tax rate). Three module constants sit outside the templates and are documented inline in §4.1.4: `_BECKER_TOMES_RANK_NOISE_SD = 0.75`, `MOURNING_MEMORY_WEIGHT = 0.9`, `MOURNING_TIE_STRENGTH_THRESHOLD = 0.6`. The era-noise priors `DEFAULT_ERA_MEAN = 0.5` and `DEFAULT_ERA_SD = 0.15` are nominally placeholders but are in practice the parameters, since no template declares an `era_noise` section — see the Simplifications paragraph of §4.1.4, which records that the 2026-08-07 amendment makes the section mandatory at the same values, after the pinned commit. |
...
deferred to a separate work item with its own requirements gate, and that
work item has since corrected all six. They are kept here, marked, because
results published before the correction are not comparable with results
published after it.
- FIXED. The polygenic kernel was not variance-preserving: equation (4.46)
  was a convex combination rather than the Falconer and Mackay
  decomposition, and measured trait spread collapsed to 48.8% of the
  declared era distribution within roughly three generations. The three
  residual scales now follow from the variance identity.
- FIXED. The single-parent fallback did not halve the genetic signal
  despite the spec and the docstring claiming it did; the implemented
  coefficient was `h²` where Falconer gives `h²/2`, so the implemented
  resemblance was twice the cited model's.
- FIXED. The shari'a spouse fraction was gender-blind. Q4:12 gives the
  widower 1/2 without a child and 1/4 with, the widow 1/4 and 1/8; the
  implementation applied the widow's schedule to both partners.
- FIXED. Estate tax and remainder were two independent products, so exact
  conservation failed on 16.1% of estates at rate 0.15 and 6.0% at 0.40.
  The smaller term is now derived by subtraction on either side of a rate
  of one half, which is Sterbenz's condition, and the property holds
  bit-for-bit over the whole rate domain in the stated summation order.
- FIXED. The `education_regression_rho` values traced to an attribution
  that does not exist: no Chetty et al. (2014) paper reports an education
  persistence coefficient of any value. The five templates now ship
  0.60 / 0.60 / 0.60 / 0.60 / 0.30, anchored on Black and Devereux (2011,
  Table 3) for the modern era and declared tunable heuristics elsewhere.
- FIXED. The era-noise priors `DEFAULT_ERA_MEAN = 0.5` and
  `DEFAULT_ERA_SD = 0.15` were documented placeholders that were in
  practice the parameters, since no template declared an `era_noise`
  section. All five now declare it, at the same values, so the numbers
  are unchanged and are explicit.
...
three. They are kept, marked, because results published before and after
the correction are not comparable.
- FIXED. The flight trigger's threshold was described as a global `N`
  that the design defined as "the number of ticks the agent can survive
  on current savings" -- which reduces the condition to `wealth <
  wealth`, confusing a quantity with a threshold on it. The survival
  horizon is now a derived ratio with no global threshold; emergency
  flight declares 1 and leaves duration to its per-era
  `flight_trigger_ticks`, fertility consumes it continuously. Neither
  consumer's behaviour changed: the incoherence was in the document.
- FIXED. Equation (4.57) was dimensionally inconsistent, subtracting a
  count of ticks from a per-tick currency rate. It is now a discounted
  present value (Todaro 1969, Sjaastad 1962) in which every term is
  money. The declared cost of the correction is large: the break-even
  distance moves from 4.8 ticks to 220.5 and distance effectively stops
  binding, which is what the cited model implies and what its own author
  writes down.
- FIXED. "Zone stability" was a simulation-wide scalar reported
  identically inside every zone's entry, unlabelled. It is now reported
  once at outlook level and named as a simulation value. Declared limit:
  the migration decision cannot discriminate between zones on stability.
- Only the earnings-foregone half of Sjaastad's cost is modelled, since
  nothing in the schema prices a journey in currency; an agent whose
  zone has no wage data therefore travels for free.
- Migration stops entirely at age 62: the residual working life reaches
  zero rather than tapering, so the annuity is exactly 0.0 and no wage
  differential can produce a positive gain. An agent at or past that age
  cannot flee starvation toward any destination. Correct under the
  investment framing, unendorsed by any source at that exact age.
- The Clark class rule and the education regression carry innovation
  terms as of amendment A3. Without them both were pure contractions:
  education reached a fixed point of ZERO rather than a reduced
  dispersion, and the class rule froze every lineage on the zone mean at
  a measured mobility of exactly 0.0000 and a parent-child rank
  correlation of exactly 1.0000 from the second generation, with two of
  the five ranks empty. The Clark amplitude is SOLVED against the
  realized dispersion of the rounded, clamped five-state chain rather
  than read off the stationary identity, which overshoots by 2.26%.
...
- Falconer, D. S., and Mackay, T. F. C. (1996). *Introduction to
  ...
  ISBN 978-0-582-24302-6. (Chapters 8-10; chapter NUMBERS verified against
  the fourth edition's table of contents, titles deliberately omitted and
  the book itself unopened. See §4.1.4, equations (4.46)–(4.48),
