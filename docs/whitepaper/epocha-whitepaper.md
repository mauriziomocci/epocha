...
Equation (4.46) is the variance-preserving form of the Falconer and Mackay (1996, ch. 10) decomposition, and the three residual scales are **derived from one identity rather than chosen**. Requiring the offspring variance to equal the parental variance `V` under random mating, with `Var(parent_term) = V/2` at two parents and `V` at one, gives
