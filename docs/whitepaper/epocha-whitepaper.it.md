...
Il clamp a `[0, 1]` tronca la distribuzione, quindi la dispersione realizzata resta sotto l'`era_sd` dichiarata anche sotto l'identità esatta: il punto fisso su griglia deterministica in `truncated_moments.py` colloca l'ampiezza stazionaria al **91,8%** del valore dichiarato nella configurazione modale. Quella cifra è misurata e riportata al caricamento del template, non messa sotto criterio: una banda su di essa non separa un kernel corretto da uno con il residuo mal scalato, che è esattamente il fallimento per cui esistono i test esatti sulle scale.
