...
date: "2026-04-26"
version: "0.1"
frozen-at-commit: "fb893f53546355eb929f264ac3314f4bfc8a925c"
license: "Apache 2.0"
---
...
**L'orizzonte di sopravvivenza, condiviso con §4.1.5** (emendamento A8). `wealth / subsistence_threshold` è un numero puro che dice quanti tick di sussistenza coprono i risparmi di un agente, ed è una grandezza DERIVATA: non esiste alcuna soglia globale `N` su di essa in nessun punto del modello. Il documento di design ne definiva una come "il numero di tick che l'agente può sopravvivere coi risparmi attuali", il che riduce la condizione in cui compariva a `ricchezza < ricchezza` — una grandezza confusa con una soglia su di essa. Ogni consumatore ora dichiara la propria con la propria motivazione. La fertilità consuma l'orizzonte in continuo attraverso il logaritmo della (4.3), e lo `0,1` dentro quel logaritmo è una GUARDIA NUMERICA contro `log(0)` per un agente indigente, che limita il segnale a `log(0,1) = −2,30` anziché a meno infinito; non è un pavimento modellato sull'orizzonte e non va letto come parametro tarato. La fuga d'emergenza dichiara la soglia 1 e affida la durata al proprio `flight_trigger_ticks` per era. Nessuno dei due consumatori ha cambiato comportamento quando questo è stato corretto: l'incoerenza era interamente nel documento di design.
...
**Modello.** Il kernel poligenico è `inherit_trait()`:

```
child_T = μ_T + b · (parent_term_T − μ_T) + e_T                (4.46)
e_T ~ N(0, c · era_sd_T)

parent_term_T = (mother_T + father_T) / 2   entrambi i genitori risolti
              = l'unico genitore risolto     solo madre / solo padre
              = μ_T = era_mean_T             nessuno dei due

           b = h²_T,  c = √(1 − h⁴_T / 2)     entrambi i genitori
           b = h²_T / 2,  c = √(1 − h⁴_T / 4)  un solo genitore
           b = 0,  c = 1                       nessuno dei due
```

Il risultato è clampato a `[lo, hi]`, default `[0, 1]`. L'estrazione del rumore avviene esattamente una volta per chiamata e sempre *dopo* il ramo, così la sequenza RNG consumata da questa funzione non dipende da quale ramo abbia girato — una proprietà di riproducibilità deliberata, non un accidente dell'ordine delle istruzioni.

L'equazione (4.46) è la forma che preserva la varianza della decomposizione di Falconer e Mackay (1996, capp. 8-10), e le tre scale del residuo sono **derivate da una sola identità, non scelte**. Imporre che la varianza della prole eguagli quella parentale `V` sotto accoppiamento casuale, con `Var(parent_term) = V/2` a due genitori e `V` a uno solo, dà

```
V = b² · Var(parent_term) + c² · s²,   s = era_sd_T           (4.47)

due genitori:   V = h⁴ · V/2 + c² s²   ⇒  c = √(1 − h⁴/2)
un genitore:    V = (h⁴/4) · V + c² s² ⇒  c = √(1 − h⁴/4)
nessun genitore: V = c² s²              ⇒  c = 1
```

dove `h⁴` denota `(h²)²`, il quadrato dell'ereditabilità, secondo la convenzione standard della genetica quantitativa per cui `h²` è esso stesso il parametro. **Nel codice `h⁴` è `h2**2`, non `h2**4`**: la variabile contiene già il quadrato. Scrivere l'esponente due volte gonfia la deviazione standard del residuo del 6,03% e la varianza del 12,43% a `h² = 0,55` lasciando la pendenza della regressione esattamente corretta, quindi un test sulla pendenza non può vederlo; è per questo che `test_transmission_kernel.py` mette sotto criterio le scale stesse. A `h² = 0,55` e `era_sd = 0,15` il residuo a due genitori vale `0,15 · 0,921276 = 0,138191` e quello a genitore singolo `0,15 · 0,961444 = 0,144217`.

**Che cosa ha sostituito, e perché è registrato qui.** Fino alla campagna sui difetti di design dell'agosto 2026 il kernel spediva come `child_T = h²_T · midparent_T + (1 − h²_T) · ε_T` con `ε_T ~ N(era_mean_T, era_sd_T)` — trascrizione fedele della Sezione 4 della spec di design, e **non** la decomposizione che citava. Il suo residuo era scalato per `(1 − h²)` anziché per `√(1 − h⁴/2)`, che a `h² = 0,55` fornisce `0,0675` dove l'identità richiede `0,138191` — poco meno della metà — così la varianza si contraeva a ogni generazione fino a un punto fisso di `sd* = 0,0733`, il **48,8% della deviazione standard d'era da cui il modello dichiarava di campionare**. L'audit ha misurato esattamente questo simulando 4.000 agenti su otto generazioni. Il ramo a genitore singolo era la seconda divergenza: applicava l'intero `h²` all'unico genitore noto dove la fonte dà `h²/2`. Entrambe sono ora corrette anziché documentate come limitazioni, e la vecchia forma è riportata qui perché la correzione sia verificabile contro ciò che ha corretto.

Il clamp a `[0, 1]` tronca la distribuzione, quindi la dispersione realizzata resta sotto l'`era_sd` dichiarata anche sotto l'identità esatta. Quanto sotto dipende da dove sta la coppia: il punto fisso su griglia deterministica in `truncated_moments.py` colloca l'ampiezza stazionaria al **99,91%** del valore dichiarato sulla coppia dei tratti centrata `(0,50, 0,15)` e al **97,72%** su quella dell'istruzione fuori centro `(0,30, 0,15)`, che porta anche il 2,14% della massa sui bordi. Quelle cifre sono misurate e riportate al caricamento del template, non messe sotto criterio: una banda sull'ampiezza realizzata non separa un kernel corretto da uno con il residuo mal scalato, che è esattamente il fallimento per cui esistono i test esatti sulle scale. Il pavimento che il caricatore impone davvero è il 95%, sul peggiore dei tre rami di parentela e non su uno convenzionale, perché misurato su una griglia di configurazioni il ramo a due genitori non è il minimo nella maggioranza dei casi.

**L'identità assume accoppiamento casuale, e il modello non lo garantisce.** `Var(midparent) = V/2` vale solo per genitori scorrelati, e il punteggio di omogamia pesa l'istruzione fra 0,25 e 0,40 in tutte e cinque le ere — quindi restituire dispersione all'istruzione è precisamente ciò che risveglia quella correlazione, ovunque e non solo dove una regola di classe legge un tratto. Con genitori correlati a `r` la varianza del midparent è `V(1+r)/2` e l'ampiezza stazionaria si gonfia a `√((1 − b²/2) / (1 − b²(1+r)/2))` del valore dichiarato. Sulla ricorsione non troncata questo esce dalla banda 95-105% oltre `r = 0,5217` per i tratti e `r = 0,4235` per l'istruzione; **il troncamento inverte l'ordine**, spostando gli attraversamenti a `r = 0,5403` e `r = 0,7122`, perché la coppia dell'istruzione fuori centro deve prima recuperare il 2,28% che il clamp le costa già. Quei quattro numeri sono riportati accanto alla `r` misurata, mai messi sotto criterio: servono a rendere visibile quanto l'accoppiamento abbia spostato la varianza, non a bocciare un template. La convenzione di accoppiamento è dichiarata anziché lasciata all'implementatore — copula gaussiana sulla marginale corrente, con la correlazione di Pearson realizzata sulla scala osservata `[0, 1]` come grandezza riportata, dato che la massa che il clamp accumula su un bordo è appaiata e non porta covarianza, quindi il parametro di copula sovrastimerebbe l'assortimento. Due implementazioni che scegliessero convenzioni diverse pubblicherebbero soglie diverse per la stessa era: è per questo che la scelta è fissata qui.
...
dove `parent_rank` è il rango del *padre* sulla scala `elite=0 … poor=4, enslaved=5`, con fallback sulla madre e poi su `working` (`_resolve_parent_rank()`, `inheritance.py:859`), `zone_class_mean` è il rango medio della popolazione vivente della zona della madre, e l'output di ogni regola campionata è clampato a `[0, 4]` prima dell'arrotondamento a etichetta. Quel tetto sull'output è un fix d'audit: prima di esso, l'ordinaria aritmetica media-pesata-più-rumore sotto `becker_tomes` arrotondava al rango 5 — `enslaved` — per il **25,25%** dei figli di due genitori `poor` in una zona interamente `poor`, in `modern_democracy`, l'unica era in cui la schiavitù non deve mai comparire. `enslaved` sopravvive come rango in *ingresso* e raggiunge un figlio per esattamente una via: la copia testuale di `patrilineal_rigid` da un padre già schiavo, che non passa affatto per l'arrotondamento.

L'innovazione `N(0, σ_rank)` nella (4.49) non è un ornamento e la sua assenza non era una semplificazione. Il modello di Clark è un AR(1) *con* innovazione, `x_t = b·x_{t−1} + e_t` (Clark, Cummins, Hao e Diaz Vidal 2014, eq. 3-4); senza `e_t` ogni lignaggio cammina monotonicamente verso la media di zona e la varianza trasversale svanisce, comportamento asintotico opposto a quello che la fonte descrive — la sua bassa mobilità è regressione lenta, non congelamento. Misurato sulla forma spedita fino all'agosto 2026: mobilità intergenerazionale esattamente 0,0000 e correlazione di rango genitore-figlio esattamente 1,0000 dalla seconda generazione, su una partizione con due dei cinque ranghi vuoti. L'identità stazionaria `σ = s_rank · √(1 − b²)` fissa l'ampiezza su scala continua, ma l'arrotondamento a etichette intere e il clamp a entrambi gli estremi sono non lineari, quindi quella lettura eccede: al `target_dispersion = 1.0` dichiarato realizza 1,0226. `clark_calibration.solve_clark_innovation()` perciò *risolve* l'ampiezza contro la dispersione realizzata della catena a cinque stati arrotondata e clampata, per bisezione a 1e-12, ottenendo `σ_rank = 0,688956` — il 96,47% della lettura ingenua — e una mobilità realizzata di 0,4993. La regressione dell'istruzione è (`inheritance.py:961`):

```
child.education = la (4.46) con b, c ricavati da ρ al posto di h²        (4.50)
```

cioè l'istruzione passa per lo *stesso* kernel dei tratti, con il coefficiente di regressione d'era `ρ` nel ruolo di `h²` e con `era_mean_education`, `era_sd_education` dichiarati per era sotto `social_inheritance.era_noise.education`. È letteralmente la stessa chiamata, non un'analogia.

Fino all'agosto 2026 questo passo era `ρ · midparent_education + (1 − ρ) · era_mean_education`, una contrazione puramente deterministica senza alcun termine casuale, e il suo punto fisso non è una dispersione ridotta ma **zero**: misurato su otto generazioni da una dispersione iniziale di 0,150, `ρ = 0,5` arriva a 0,00004 e `ρ = 0,2` a zero esatto, rendendo l'istruzione una costante. La conseguenza si propagava, perché la regola di classe `meritocratic` legge l'istruzione e il punteggio di omogamia la pesa tra 0,25 e 0,40. Anche la media d'era era una costante di modulo pari a 0,3 che nessun template dichiarava; ora la dichiarano tutti e cinque. Gira prima del dispatch di classe così che `meritocratic` — l'unica regola che legge `child.education_level` — veda il valore regredito anziché il default di campo di `Agent`; eseguirla per seconda sottostimava il merito di 0,06 e il rango finale di 0,19 con due genitori di istruzione 0,9 sotto `sci_fi`, abbastanza da retrocedere un figlio di una classe intera.

La liquidazione dell'asse ereditario si scompone in uno step di imposta e uno di allocazione:

```
rate <= 1/2 :  inheritable = total_estate · (1 − rate)          (4.51)
               tax_revenue = total_estate − inheritable
rate >  1/2 :  tax_revenue = total_estate · rate
               inheritable = total_estate − tax_revenue
```
...
shari'a       : vedovo 1/4 con figli, 1/2 senza;
                vedova (e coniuge non binario) 1/8 e 1/4
...
|---|---|---:|---|---:|
| `pre_industrial_christian` | `patrilineal_rigid` | 0,60 | `primogeniture` | 0,00 |
| `pre_industrial_islamic` | `patrilineal_rigid` | 0,60 | `shari'a` | 0,00 |
| `industrial` | `clark_regression` | 0,60 | `equal_split` | 0,15 |
| `modern_democracy` | `becker_tomes_elasticity_0.4` | 0,60 | `equal_split` | 0,40 |
| `sci_fi` | `meritocratic` | 0,30 | `equal_split` | 0,00 |

Due fatti sulla Tabella 4.11 non vanno fraintesi. Primo, i template spediti esercitano solo tre delle cinque regole di successione implementate: `primogeniture` una volta, `shari'a` una volta ed `equal_split` tre volte. `matrilineal` e `nationalized` sono implementate, testate separatamente e dichiarate da nessun template; esistono per futuri template personalizzati e per completezza dei cinque sistemi documentati. Secondo, la colonna ρ porta i valori che l'emendamento A11 ha ancorato a Black e Devereux (2011, Tabella 3): la regressione congiunta di Plug (2004) dà 0,30 sul padre e 0,30 sulla madre, e il coefficiente confrontabile con un ρ applicato al valore medio dei genitori è la loro SOMMA, 0,60. Quattro ere condividono deliberatamente quel valore unico — far variare ρ su una progressione che nessuna fonte sostiene darebbe l'apparenza di una serie storica misurata — e solo `modern_democracy` è ancorata; le altre tre sono dichiarate euristiche tarabili. Lo 0,30 di `sci_fi` è una scelta speculativa dichiarata, senza fonte e senza pretenderne una. I valori spediti prima dell'agosto 2026 erano 0,5 / 0,5 / 0,4 / 0,4 / 0,2, riconducibili a un'attribuzione che non esiste. L'aliquota di successione dello 0,40 di `modern_democracy` corrisponde alle aliquote storiche di fascia alta documentate in Piketty (2014, tabelle 14.1–14.2); lo 0,00 pre-industriale non è l'affermazione che le élite pre-industriali non pagassero alcun tributo di morte, ma una dichiarazione di perimetro — i pagamenti feudali di rilievo e i prelievi analoghi sul trasferimento di potere appartengono allo strato economia, non a questa voce.

**Algoritmo.** Il percorso di nascita è `apply_inheritance_at_birth(child, mother, father, simulation, tick)` (`inheritance.py:1115`). Carica il template d'era, calcola `zone_class_mean` sulla zona della madre in una sola query `values_list`, estrae un unico `random.Random` da `get_seeded_rng(simulation, tick, phase="inheritance")` ed esegue tre step contro quell'unico stream condiviso in ordine fisso: `apply_trait_inheritance()` percorre i tratti ereditabili nell'ordine di inserimento della `heritability` del template, mai un `set` nudo, perché `rng.gauss` è estratto una volta per tratto e un'iterazione non ordinata farebbe dipendere la sequenza di estrazioni dall'hash seed di stringa del processo. L'insieme trasmesso è esattamente quello dichiarato: fino all'agosto 2026 la scansione veniva estesa con qualunque chiave di personalità presente su un genitore ma assente da `heritability`, il che dava silenziosamente a un tratto non dichiarato l'ereditabilità `default` e faceva dipendere l'insieme trasmesso dai genitori anziché dall'era. Quella chiave `default` è stata rimossa dallo schema, e un template che la dichiari viene ora rifiutato; `resolve_birth_attributes()` consuma esattamente due estrazioni `rng.random()` per genere e orientamento sessuale, il genere dal rapporto secondario dei sessi via `p_male = ratio / (1 + ratio)` (1,05 in ogni template tranne l'1,0 di `sci_fi`, e strutturalmente incapace di produrre `non_binary`, dato che il rapporto secondario dei sessi è una statistica di sesso alla nascita e non una distribuzione di identità di genere), l'orientamento con una scansione cumulativa sulla `sexual_orientation_distribution` del template nell'ordine di inserimento JSON, il cui default moderno — heterosexual 0,955, bisexual 0,030, homosexual 0,015 — proviene da Chandra et al. (2011) ed è portato come parametro di design tunable per le ere in cui non esiste un'indagine comparabile, non come pretesa di universalità; e `apply_social_inheritance()` esegue la (4.50) e poi fa dispatch della (4.49). **L'1,05 del rapporto dei sessi è un parametro di progetto dichiarato e senza fonte.** Il modulo lo attribuiva a Falconer e Mackay, per averlo definito una delle costanti meglio documentate della genetica umana; quello è un testo di genetica quantitativa e il materiale qui citato non dice nulla sul rapporto secondario dei sessi umano, quindi l'attribuzione è ritirata in attesa di una fonte demografica adeguata. La cifra resta, tarabile per era. La `wealth` del figlio è posta incondizionatamente a 0.0 e la `zone` a quella della madre. Nulla viene salvato: la funzione muta `child` e solo `child`, lasciando la persistenza all'orchestratore di nascita del Plan 4.
...
**Semplificazioni.** Quattro round di audit adversariale del codice sono convergiti il 2026-08-05 su `inheritance.py`, `migration.py` e la modifica a `couple.py` di §4.1.3. Quel verdetto copriva il codice come delimitato e non copriva esplicitamente i difetti di livello PROGETTUALE, rinviati a un work item separato con un proprio gate di requisiti. Quel work item ha poi consegnato le correzioni qui sotto; ciascuna è dichiarata insieme a cio' che ha sostituito, perche' un capitolo che scambiasse in silenzio un'affermazione con un'altra lascerebbe senza marcatura ogni risultato pubblicato sotto la precedente.

**I risultati prodotti prima e dopo queste correzioni non sono comparabili.** Sono cambiati il kernel dei tratti, la regressione dell'istruzione, la regola di classe dell'era industriale, la quota coniugale dell'era islamica e quattro dei cinque coefficienti dell'istruzione. Qualunque cifra multi-generazionale precedente descrive un modello diverso.

*Il kernel poligenico ora preserva la varianza.* Fino all'agosto 2026 l'equazione (4.46) calcolava una combinazione convessa anziché la decomposizione di Falconer e Mackay che citava, e la dispersione dei tratti collassava al 48,8% della distribuzione d'era dichiarata entro circa tre generazioni e vi restava — ogni società simulata andava verso l'omogeneità su tutti e tredici i tratti ereditabili della Tabella 4.10, con l'ereditabilità realizzata che smetteva di corrispondere alle cifre citate dopo la prima generazione. Le tre scale del residuo ora seguono dall'identità di varianza, e il ramo a genitore singolo applica `h²/2` anziché l'intero `h²`: il vecchio ramo restituiva un valore bit-identico per due genitori a 0,9 e per la sola madre a 0,9, una somiglianza implementata doppia di quella del modello citato.

*La quota coniugale shari'a è per genere.* Q4:12 è asimmetrico — il vedovo prende 1/2 senza figli e 1/4 con figli, la vedova 1/4 e 1/8 — e l'implementazione applicava a entrambi la scaletta della vedova, quindi un marito superstite riceveva metà del suo diritto classico in entrambi i rami. Le frazioni sono ora lette sul testo primario; Powers (1986) resta la fonte della struttura quota-fissa-più-residuo e del rapporto 2:1 di Q4:11, entrambi verificati corretti, e non è mai stato fonte della quota cieca al genere.

*Imposta di successione e residuo si conservano esattamente.* L'equazione (4.51) moltiplicava due volte anziché derivare un termine per differenza, e la conservazione esatta `imposta + residuo == totale` falliva sul 16,1% dei patrimoni ad aliquota 0,15 e sul 6,0% a 0,40, con errore relativo di 1,9e-16. Ora deriva per differenza il termine MINORE su ciascun lato dell'aliquota un mezzo, che è la condizione del lemma di Sterbenz, e la proprietà regge bit per bit su tutto il dominio di aliquote — verificata a zero fallimenti su 20.000 patrimoni per ciascuna di undici aliquote, con l'imposta letta dal tesoro anziché riderivata. La garanzia dichiara il proprio ordine di somma: `inheritable + tax_revenue`. Derivare il solo residuo non raggiunge l'esattezza nemmeno alle aliquote spedite; derivare la sola imposta è esatto solo fino a 0,5 e poi degrada al 12,68% delle prove a 0,70.

*I coefficienti dell'istruzione risalgono a una fonte che esiste.* Il design attribuiva il valore moderno a Chetty et al. (2014). I testi completi di entrambi i lavori del 2014 sono stati estratti e cercati: nessuno dei due riporta un coefficiente di persistenza dell'istruzione, di nessun valore. Quello che misura l'istruzione riporta un gradiente rispetto al RANGO DI REDDITO, 74,5% e 69,2%, non rispetto all'istruzione. Il valore è stato rimosso anziché ricollocato — la trappola da nominare è che il coefficiente materno di Sacerdote (2000) vale anch'esso 0,35, ma è a genitore singolo e non è Chetty. Anche l'intervallo di elasticità dello stesso lavoro era citato al contrario: i valori misurati vanno da 0,264 a 0,697 e la tesi è che l'elasticità sia inaffidabile, quindi `becker_tomes` ora attribuisce lo 0,4 al solo Solon (1999), con stato di verifica dichiarato NON VERIFICATO perché quel capitolo è dietro paywall e non è stato letto.
...
> Stato: implementato a partire dal commit `1cdcfa4fe23138727c16a2e92234e4eb962d9ae7`, audit della spec CONVERGENTE 2026-04-18 round 4, audit del codice CONVERGENTE 2026-08-05 round 4 **come delimitato**; i difetti di design rinviati sono dichiarati sotto Semplificazioni.
...
```
E[gain_j] = a(H, r) · [ (1 − unemployment_j) · wage_j − wage_current ]
            − distance_cost_ticks_j · wage_current                       (4.57)

a(H, r) = (1 − e^(−r·H)) / r          [tick]
H       = max(0, 62 − eta) · tick_per_anno,  tick_per_anno = 8760 / tick_duration_hours
r       = 0,10 / tick_per_anno
```

L'equazione (4.57) è un **valore attuale scontato**, e le fonti dietro di essa si separano nettamente. Harris e Todaro (1970) è un'uguaglianza di equilibrio a un solo periodo, `W_u·E_u/L_u = W_R`, senza orizzonte, senza sconto e senza costo: è la fonte della ponderazione per probabilità d'impiego sul salario di destinazione e di nient'altro — in particolare non può licenziare un orizzonte. Todaro (1969) enuncia la decisione come `V(0) = Σ [p(t)·Y_u(t) − Y_r(t)]·e^(−it) − C(0)`, un costo una tantum sottratto a un flusso scontato anziché compensato contro un tasso. Sjaastad (1962) fornisce sia l'orizzonte — vita lavorativa residua, che si accorcia con l'età — sia la definizione del costo: esborsi più guadagni mancati mentre si viaggia e si cerca lavoro, in parte funzione della distanza. Il `62` non è scelto: è l'età da cui discendono entrambi gli orizzonti che Sjaastad pubblica, 45 anni residui per la fascia 15-19 e 40 per la 20-24, applicati ai punti medi delle fasce. Il tasso di sconto del 10% annuo è la sua assunzione dichiarata (p.92), qui tarabile; con un tick pari a un anno l'annualità riproduce i due fattori che stampa a p.89, 9,889 e 9,817.

Fino all'agosto 2026 l'equazione era `(1 − unemployment_j) · wage_j − wage_current − distance_cost_j`, che sottrae un **conteggio di tick** da una **moneta per tick**. L'esempio numerico del design non lo espone, perché la destinazione che calcola ha costo di distanza nullo. Nemmeno monetizzare il solo costo risolve: lascia una moneta contro due tassi. Due scostamenti dal modello canonico a due settori restano documentati anziché taciuti: il salario del settore informale è posto a zero, quindi un agente che non trova lavoro formale a destinazione non guadagna nulla lì, e viene portato un termine di distanza che il modello a due settori non ha.

`build_migration_outlook()` (`migration.py:456`) assembla il blocco di prompt per-agente da un bundle `zone_stats` precalcolato una volta per tick, senza emettere alcuna query propria. Ogni zona raggiungibile — definita come ogni zona del mondo diversa da quella dell'agente, senza vincolo di raggio, perché lo schema non porta alcun concetto di distanza massima di viaggio e la (4.56) già prezza la distanza — riceve una voce che porta `wage_differential`, `unemployment`, `distance_cost` ed `expected_gain`, con `government_stability` riportato una volta sola a livello di outlook. Il coordinamento familiare è `coordinate_family_migration()` (`migration.py:681`): il partner vivente dell'agente decisore e ogni figlio vivente sotto l'`adulthood_age` dell'era sono spostati nella zona di destinazione nello stesso tick, con `location` scritta accanto a `zone` in un solo `bulk_update`, e un solo `DemographyEvent` di tipo `migration` è emesso per l'intero nucleo anziché uno per membro. I figli adulti sono deliberatamente esclusi — decidono in modo indipendente — e nessun minore riceve mai una riga di `DecisionLog` per uno spostamento che non ha scelto.
...
**Semplificazioni.** I due difetti di livello progettuale che l'audit di codice convergente aveva lasciato aperti sono stati corretti entrambi dal work item rinviato, e **i risultati prodotti prima e dopo non sono comparabili** — il guadagno atteso è cambiato di tre ordini di grandezza, e con esso il verdetto sugli agenti intrappolati.

*Il guadagno atteso è un valore attuale.* La vecchia forma era dimensionalmente incoerente, e la conseguenza della correzione è grande ed è una perdita, quindi va dichiarata anziché sepolta. Con tick di 24 ore, orizzonte di 40 anni e 10% annuo, `a` vale 3583 tick, quindi l'esempio numerico del design di +4,8 per tick diventa un valore attuale di +17.199. Il costo distanza di pareggio passa da 4,8 tick a 220,5, contro costi spediti di 0, 3 e 5: **la distanza cessa di mordere**, e la soglia si allarga di circa un fattore quarantacinque. È ciò che il modello citato implica, e lo scrive il suo stesso autore — Sjaastad osserva (p.84) che i costi marginali per miglio dovrebbero essere altissimi per conciliare l'effetto della distanza osservato con il valore attuale del differenziale, "anche a tassi di sconto molto elevati". Il modello di investimento sotto-predice l'attrito della distanza.

*La migrazione si ferma del tutto a 62 anni.* L'orizzonte raggiunge lo zero anziché assottigliarsi, quindi da `WORKING_LIFE_END_AGE` in poi l'annualità vale esattamente 0,0 e la (4.57) collassa a `−distance_cost_ticks · wage_current`: zero verso una destinazione co-locata e strettamente negativo altrimenti, per quanto grande sia il differenziale salariale. Poiché il percorso di fuga rifiuta un guadagno non positivo, un agente a quell'età o oltre è permanentemente immobile e non può fuggire dalla fame nemmeno verso una zona che paghi mille volte la sua. Misurato: il fattore vale 347,3 tick a 61 anni e 0,0 a 62. È aritmeticamente corretto sotto l'inquadramento di Sjaastad come investimento — chi non ha più vita lavorativa non recupera nulla da uno spostamento — ed è un confine netto che nessuna fonte avalla a quell'età esatta. Il `max(0, ·)` nell'orizzonte sopra è portante e non decorativo: senza di esso un settantenne porterebbe un'annualità negativa e verrebbe PAGATO per migrare in quanto vecchio. Il raffinamento naturale, rinviato anziché inventato, è un movente non lavorativo per la migrazione tardiva che non passi affatto per il differenziale salariale; il modello citato non ha nulla da dire in proposito.

*Solo una delle due componenti di costo di Sjaastad è modellata.* Il suo costo è esborsi PIÙ guadagni mancati; la (4.57) prezza solo la seconda, perché `compute_distance_cost` restituisce tick e nulla nello schema prezza un viaggio in valuta. La conseguenza è netta in fondo alla distribuzione: un agente la cui zona non ha dati salariali ha `wage_current = 0`, quindi viaggiare è gratis e nessuna destinazione viene mai scartata sulla distanza. Corretto sotto la componente modellata — chi non guadagna nulla non rinuncia a nulla partendo — e sbagliato sotto quella non modellata, che è esattamente la popolazione a cui un esborso vivo pesa di più. Trovato ricostruendo le fixture di agente intrappolato di questo stesso modulo contro la nuova equazione, non per ispezione.

*La soglia del trigger di fuga è derivata anziché globale.* L'**orizzonte di sopravvivenza** è `agent.wealth / compute_subsistence_threshold(simulation, zone)`, un numero puro che dice quanti tick di sussistenza coprono i risparmi. Non esiste alcun `N` globale: il design ne definiva uno come "il numero di tick che l'agente può sopravvivere coi risparmi attuali", il che riduce la condizione in cui compariva a `ricchezza < ricchezza`, confondendo una grandezza con una soglia su di essa. Ogni consumatore dichiara la propria. La fuga d'emergenza applica la soglia **1** — incapacità di coprire anche un solo tick — e affida la durata al proprio `flight_trigger_ticks` per era, che vale 30, 30, 20, 10 e 5: imporre un secondo orizzonte anche sul livello conterebbe due volte la stessa durata, con la seconda occorrenza priva di fonte. La fertilità consuma l'orizzonte in forma logaritmica continua con un pavimento a 0,1 che è una guardia numerica contro `log(0)`, non un pavimento modellato. Nessuno dei due consumatori ha cambiato comportamento: l'incoerenza era interamente nel documento di design.
...
> Stato: implementato a partire dal commit `1cdcfa4fe23138727c16a2e92234e4eb962d9ae7`, audit del codice CONVERGENTE 2026-07-16 round 12.
...
> Stato: implementato a partire dal commit `1cdcfa4fe23138727c16a2e92234e4eb962d9ae7`, audit del codice CONVERGENTE 2026-07-16 round 12. Attivo nella pipeline per tick (dispatched da `epocha/apps/simulation/engine.py` per le simulazioni con il data layer economico inizializzato).
...
| Ereditarietà (kernel poligenico + trasmissione sociale + successione) | Tabelle 4.10 (ereditabilità per tratto con fonti primarie) e 4.11 (regola di classe, ρ di regressione dell'istruzione, regola di successione e aliquota di successione per epoca). Tre costanti di modulo stanno fuori dai template e sono documentate inline in §4.1.4: `_BECKER_TOMES_RANK_NOISE_SD = 0.75`, `MOURNING_MEMORY_WEIGHT = 0.9`, `MOURNING_TIE_STRENGTH_THRESHOLD = 0.6`. I prior di rumore d'era `DEFAULT_ERA_MEAN = 0.5` e `DEFAULT_ERA_SD = 0.15` sono nominalmente segnaposto ma in pratica sono i parametri, dato che nessun template dichiara una sezione `era_noise` — vedere il paragrafo Semplificazioni di §4.1.4, che registra come l'emendamento del 2026-08-07 renda la sezione obbligatoria agli stessi valori, dopo il commit pinnato. |
...
I template di demografia sono cinque file JSON sotto `epocha/apps/demography/templates/`: `pre_industrial_christian.json`, `pre_industrial_islamic.json`, `industrial.json`, `modern_democracy.json` e `sci_fi.json`. Ciascun file porta un dizionario piatto le cui chiavi sono consumate da un modello specifico di §4.1: `mortality`, `fertility` e `couple` per §4.1.1-§4.1.3, `trait_inheritance`, `social_inheritance` ed `economic_inheritance` per §4.1.4 — le prime due portano ciascuna una sottosezione `era_noise` che dichiara, per epoca e per carattere, media e ampiezza della distribuzione osservata che il kernel di trasmissione deve realizzare — `migration` per §4.1.5, più le voci a livello di simulazione `acceleration`, `max_population`, `fertility_agency`, `age_pyramid`, `sex_ratio_at_birth` e `sexual_orientation_distribution` condivise tra i modelli. La coppia pre-industriale è una scissione deliberata: i due file condividono blocchi di mortalità e fertilità identici (perché il record storico empirico non giustifica una differenziazione per confessione negli schedule biologici sottostanti) e differiscono solo nel blocco `couple`, dove `pre_industrial_islamic.json` porta `marriage_market_type: arranged` contro il regime autonomo di tutti gli altri template e `pre_industrial_christian.json` porta `divorce_enabled: false` per modellare il regime canonico cattolico di indissolubilità del matrimonio. Lo schema JSON è stretto, e il caricatore applica sei clausole al momento del caricamento (`template_loader.py:validate_template`). Respinge: ogni chiave sconosciuta, a qualunque livello di annidamento; ogni valore fuori intervallo, con aliquote, ereditabilità e coefficienti di regressione confinati in `[0, 1]` e ampiezze richieste strettamente positive; ogni sezione annidata obbligatoria mancante, fra cui `trait_inheritance.era_noise` e `social_inheritance.era_noise`; ogni voce mancante, cosicché ogni chiave di `trait_inheritance.heritability` porti la propria voce in `trait_inheritance.era_noise` e una sezione presente ma incompleta sia rifiutata nominando ciò che manca; ogni coppia `(era_mean, era_sd)` dichiarata che cada fuori dalla regione ammissibile del kernel di trasmissione, calcolata dal punto fisso deterministico su griglia descritto in §4.1.4; e ogni incoerenza fra `heritability` ed `era_noise`, in entrambe le direzioni. Ogni rifiuto nomina il campo con il percorso completo e, dove ne esiste uno, l'intervallo ammesso. È permesso esattamente un campo non tipizzato, mai consumato: un `_note` facoltativo che porta testo libero per il lettore, usato da due dei cinque template.

**Questo paragrafo affermava in precedenza quella proprietà e l'affermazione era falsa, il che va detto anziché corretto in silenzio.** Verificato eseguendo il validatore contro un singolo template costruito ad arte: ha accettato una chiave di primo livello inventata, una sezione `era_noize` con un refuso, un `estate_tax_rate` pari a 40 anziché 0,40, un'ereditabilità di 5,0 e un coefficiente di regressione dell'istruzione negativo — tutti e cinque insieme e senza protestare. Nessun controllo di chiave sconosciuta, di tipo o di intervallo esisteva nel caricatore. Il contratto sopra è stato specificato dalla clausola A9 dell'emendamento alla spec di design del 2026-08-07 e implementato sul ramo che porta quell'emendamento. Quel ramo è ora mergiato e **il pin frozen-at-commit è stato risolto di nuovo sul commit di merge**, quindi chi faccia il checkout del commit pinnato trova il caricatore con questi controlli. La frase che stava qui diceva che il pin puntava ancora allo stato pre-emendamento, ed è il merge ad averla resa falsa: lo annoto invece di sostituirla in silenzio, perché la prosa che descrive uno stato superato è l'unica classe di difetto che il gate di fase 6 ha colto undici volte.
...
Erano rinviate a un work item separato con un proprio gate di requisiti, e
quel work item le ha poi corrette tutte e sei. Restano qui, marcate, perché
i risultati pubblicati prima della correzione non sono comparabili con
quelli pubblicati dopo.
- CORRETTO. Il kernel poligenico non preservava la varianza: l'equazione
  (4.46) era una combinazione convessa anziché la decomposizione di
  Falconer e Mackay, e la dispersione misurata dei tratti collassava al
  48,8% della distribuzione d'era dichiarata entro circa tre generazioni.
  Le tre scale del residuo ora seguono dall'identità di varianza.
- CORRETTO. Il fallback a genitore singolo non dimezzava il segnale
  genetico nonostante spec e docstring affermassero che lo facesse: il
  coefficiente implementato era `h²` dove Falconer dà `h²/2`, quindi la
  somiglianza implementata era doppia di quella del modello citato.
- CORRETTO. La quota coniugale shari'a era cieca al genere. Q4:12 dà al
  vedovo 1/2 senza figli e 1/4 con, alla vedova 1/4 e 1/8; l'implementazione
  applicava a entrambi la scaletta della vedova.
- CORRETTO. Imposta di successione e residuo erano due prodotti
  indipendenti, quindi la conservazione esatta falliva sul 16,1% dei
  patrimoni ad aliquota 0,15 e sul 6,0% a 0,40. Il termine minore è ora
  derivato per differenza su ciascun lato dell'aliquota un mezzo, che è la
  condizione di Sterbenz, e la proprietà regge bit per bit su tutto il
  dominio nell'ordine di somma dichiarato.
- CORRETTO. I valori di `education_regression_rho` risalivano a
  un'attribuzione che non esiste: nessun lavoro di Chetty et al. (2014)
  riporta un coefficiente di persistenza dell'istruzione, di nessun valore.
  I cinque template ora spediscono 0,60 / 0,60 / 0,60 / 0,60 / 0,30,
  ancorati a Black e Devereux (2011, Tabella 3) per l'era moderna e
  dichiarati euristiche tarabili altrove.
- CORRETTO. I prior di rumore d'era `DEFAULT_ERA_MEAN = 0.5` e
  `DEFAULT_ERA_SD = 0.15` erano segnaposto documentati che in pratica erano
  i parametri, dato che nessun template dichiarava una sezione `era_noise`.
  Ora la dichiarano tutti e cinque, agli stessi valori, quindi i numeri sono
  invariati e sono espliciti.
...
- CORRETTO. L'equazione (4.57) era dimensionalmente incoerente, sottraeva un conteggio di tick da un tasso in valuta per tick. Ora è un valore attuale scontato (Todaro 1969, Sjaastad 1962) in cui ogni termine è moneta. Il costo dichiarato della correzione è grande: la distanza di pareggio passa da 4,8 tick a 220,5 e la distanza smette di fatto di vincolare, che è ciò che il modello citato implica e che il suo stesso autore mette per iscritto.
- CORRETTO. La "stabilità di zona" era uno scalare valido per l'intera simulazione riportato identico dentro l'entry di ogni zona, senza etichetta. Ora è riportato una volta sola a livello di outlook e nominato come valore di simulazione. Limite dichiarato: la decisione migratoria non può discriminare le zone sulla stabilità.
- Solo la metà "guadagni mancati" del costo di Sjaastad è modellata, dato che nulla nello schema prezza un viaggio in valuta; un agente la cui zona non ha dati salariali viaggia quindi gratis.
- La migrazione si ferma del tutto a 62 anni: la vita lavorativa residua raggiunge lo zero anziché assottigliarsi, quindi l'annualità vale esattamente 0,0 e nessun differenziale salariale può produrre un guadagno positivo. Un agente a quell'età o oltre non può fuggire dalla fame verso alcuna destinazione. Corretto sotto l'inquadramento come investimento, non avallato da alcuna fonte a quell'età esatta.
- La regola di classe alla Clark e la regressione dell'istruzione portano termini di innovazione dall'emendamento A3. Senza di essi erano entrambe contrazioni pure: l'istruzione raggiungeva un punto fisso pari a ZERO anziché una dispersione ridotta, e la regola di classe congelava ogni lignaggio sulla media di zona con mobilità misurata di esattamente 0,0000 e correlazione di rango genitore-figlio di esattamente 1,0000 dalla seconda generazione, con due dei cinque ranghi vuoti. L'ampiezza di Clark è RISOLTA contro la dispersione realizzata della catena a cinque stati arrotondata e clampata anziché letta sull'identità stazionaria, che eccede del 2,26%.
...
- Falconer, D. S., and Mackay, T. F. C. (1996). *Introduction to
  ...
  ISBN 978-0-582-24302-6. (Capitoli 8-10; i NUMERI dei capitoli sono
  verificati sull'indice della quarta edizione, i titoli sono deliberatamente
  omessi e il libro non è stato aperto. Si veda il §4.1.4, equazioni
  (4.46)–(4.48), per la divergenza documentata fra questa fonte e
  l'implementazione, e la sua correzione.)
...
questo documento — attualmente `1cdcfa4fe23138727c16a2e92234e4eb962d9ae7` e risolto al merge
