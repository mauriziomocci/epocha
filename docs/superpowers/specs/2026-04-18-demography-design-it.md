...
- risoluzione per bisezione su `σ_clark ∈ [0,075, 1,4142]`. A `s_rank = 1,0` la radice vale **0,6890**.
...
**Supera**: l'elenco dei `ρ` per era e l'attribuzione a Chetty.
...
I criteri della spec del work item non falliscono contro i modelli che devono escludere, e vanno sostituiti. **`spec.md` MUST essere emendata per recepire questa tabella**, altrimenti il gate di fase 6 sarà condotto contro criteri già noti inservibili.
...
Due condizioni sono parte del criterio e senza di esse esso **fallisce contro l'implementazione corretta**, il che sarebbe il difetto che il criterio esiste per escludere. **`|z₁ − z₂| ≥ 1`**: l'errore di arrotondamento di ciascuna valutazione è proporzionale al termine costante, non alla differenza, quindi il rapporto degrada come `ε·|segnale|/(c·s·|z₁ − z₂|)` — misurato, a `|z₁ − z₂| = 10⁻⁶` l'errore relativo vale 7,8·10⁻¹⁰ e la tolleranza è violata da un kernel corretto. **Entrambe le valutazioni MUST cadere strettamente dentro `[0,1]` prima del troncamento**, oppure l'implementazione MUST esporre il valore pre-troncamento: valutata sul valore restituito, che è clampato, la sonda sbaglia del 4-6% appena una delle due estrazioni tocca un bordo — e sul ramo senza genitori per l'istruzione, dove `T = 0,30 + 0,15·z`, `z = −2` clampa senza margine.
