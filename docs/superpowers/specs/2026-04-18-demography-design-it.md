...
Il calcolo è deterministico e si esegue una volta per coppia al caricamento: la varianza stazionaria sotto troncamento non ha forma chiusa elementare, ma si ottiene per quadratura su una griglia fissa, senza campionamento e quindi senza dimensione campionaria da dichiarare. A `(0,50, 0,15)` dà il 100,0% e passa; a `(0,80, 0,15)` dà il 92,3% e **respinge**, che è la configurazione in cui la normale accumula l'8,23% sul bordo; a `(0,50, 0,30)` dà il 90,6% e respinge.
...
Ogni coppia dichiarata MUST soddisfare i tre controlli enunciati in A1. Entrambe le coppie di questo emendamento passano, e **la seconda è stata misurata invece che assunta**: i tratti a `(0,50, 0,15)` realizzano il 100,0% dell'ampiezza con lo 0,07% di massa sul bordo; l'istruzione a `(0,30, 0,15)`, che è **una media fuori dal centro**, realizza il **98,0%** con lo 0,05% di massa sul bordo e una media stazionaria di 0,2996. Entrambe sopra la soglia del 95%.
