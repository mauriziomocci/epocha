...
- risoluzione per **bisezione su `σ_clark ∈ [0,075, 1,4142]`, con tolleranza `1·10⁻¹²` sulla dispersione e un tetto di 200 iterazioni** — servono circa 50 bisezioni per portare l'intervallo sotto `10⁻¹⁵`, e il tetto di 500 di A1 **non va trasportato qui**, perché conta iterazioni di punto fisso e non bisezioni. A `s_rank = 1,0` la radice vale **0,688956**.
...
**La successione patrilineare rigida NON riceve innovazione**: è copia deterministica dell'etichetta paterna, ed è la regola delle due ere pre-industriali perché la rigidità **è** il modello che Goody (1976) e Wrigley (1981) descrivono. **La regola meritocratica NON riceve innovazione**: deriva la classe dai tratti già ereditati del figlio, quindi riparare istruzione e intelligenza la risana da sé.
...
Due condizioni sono parte del criterio e senza di esse esso **fallisce contro l'implementazione corretta**, il che sarebbe il difetto che il criterio esiste per escludere. **`|z₁ − z₂| ≥ 1`**: l'errore di arrotondamento di ciascuna valutazione è proporzionale al termine costante, non alla differenza, quindi il rapporto degrada come `ε·|segnale|/(c·s·|z₁ − z₂|)` — misurato, a `|z₁ − z₂| = 10⁻⁶` l'errore relativo di un kernel corretto vale fra 1,3·10⁻¹¹ e 4,9·10⁻¹¹, e la tolleranza è violata; a `|z₁ − z₂| ≥ 1` lo scarto è nullo. **Entrambe le valutazioni MUST cadere strettamente dentro `[0,1]` prima del troncamento**, oppure l'implementazione MUST esporre il valore pre-troncamento: valutata sul valore restituito, che è clampato, la sonda sbaglia **senza limite superiore** appena una delle due estrazioni tocca un bordo: del 4-6% quando ne clampa una, e **del 100%** quando le clampa entrambe — sul ramo senza genitori per l'istruzione, dove `T = 0,30 + 0,15·z`, le estrazioni `z = −2` e `z = −3` restituiscono entrambe 0,0 e la sonda misura un coefficiente nullo contro `c₀ = 1`.
...
| SC-002 (verifica d'integrazione) | **SC-002b** — l'ampiezza stazionaria realizzata cade fra il 95% e il 105% del bersaglio **non corretto** `s_T`, su **almeno 5.000 individui per generazione** e otto generazioni.

**Il bersaglio NON va corretto per la correlazione fra i genitori, e la ragione è che correggerlo rende il criterio insoddisfacibile dal modello corretto.** La regione ammissibile di A1 ammette qualunque coppia la cui ampiezza realizzata sul ramo peggiore sia almeno il 95% di `s`; la perdita per troncamento cresce con l'ampiezza, quindi il rapporto rispetto a un bersaglio moltiplicato per `k(r) > 1` è **strettamente minore** di quello rispetto a `s`, e al pavimento il margine è esattamente zero. Misurato su `(0,50, 0,25)` a `h² = 0,55` — ammissibile, ramo peggiore al 95,44% — il rapporto sul bersaglio corretto vale 94,94% già a `r = 0,2` e 94,08% a `r = 0,5`: **fuori banda contro un'implementazione corretta**. La correzione di A4 resta la grandezza da **riportare** insieme a `r` (SC-017), non un cancello. La numerosità è vincolante: a 200 individui il modello **corretto** cade fuori banda 13 volte su 40, a 1.000 due volte, a 5.000 mai. I rami a un genitore e senza genitori sono sotto-popolazioni rare e vanno verificati su popolazioni sintetiche costruite per quel ramo. |
...
Perché le fonti dicono cose diverse. Goody e Wrigley descrivono una regola rigida, e la rigidità **è** il modello. Clark sostiene che lo status regredisce lentamente ma in misura strettamente non nulla, e una contrazione deterministica non lo riproduce: lo cancella.
...
**NON verificato, e nulla vi poggia**: Falconer & Mackay 1996, capitolo e pagine — il modulo cita il capitolo 8, la somiglianza fra parenti è nel capitolo 10, e il libro non è stato aperto; **A1 non dipende da quella citazione**, perché i coefficienti sono derivati per esteso nel documento. **Non verificato ma USATO, e dichiarato tale**: Solon (1999) per l'elasticità 0,4 della riga `modern`, che governa una regola viva — il capitolo è dietro paywall, non è stato letto, e l'attribuzione va confermata prima della pubblicazione; è l'unico caso in cui questo emendamento poggia su una fonte non aperta, e va risolto per primo. **Non verificati e non usati**: Hertz et al. 2008 oltre l'abstract, ed è per questo che il suo 0,4 — che è una **correlazione**, non un coefficiente di regressione — non è adottato come `ρ`; Lynch & Walsh 1998; Fisher 1918; Aitchison & Shen 1980.
